...
// @version      0.7.4
// @description  Show IMDb ratings on Douban, and vice versa; plus Letterboxd ratings everywhere
// @description:zh-CN 豆瓣和IMDb互相显示评分，同时显示Letterboxd评分
// @author       JayXon (modified)
...
// ── Letterboxd ────────────────────────────────────────────────────────────────
// Fetches Letterboxd rating and film URL for a given IMDb ID.
// Uses the /imdb/TTID/ redirect to resolve the film slug, then parses
// the film page for aggregateRating via JSON-LD (same approach as our
// other scripts). Returns { url, rating, numRaters } or null.
async function getLetterboxdInfo(imdbId) {
    return new Promise(resolve => GM.xmlHttpRequest({
        method: 'GET',
        url: `https://letterboxd.com/imdb/${imdbId}/`,
        onload: function (response) {
            // finalUrl is the resolved film page after redirect
            const finalUrl = response.finalUrl || response.responseURL || '';
            const slugMatch = finalUrl.match(/letterboxd\.com\/film\/([a-z0-9][a-z0-9-]+)\//);
            if (!slugMatch) { resolve(null); return; }

            const filmUrl = `https://letterboxd.com/film/${slugMatch[1]}/`;
                ...
                try {
                    ...
                    if (data?.aggregateRating) {
                        rating = data.aggregateRating.ratingValue
                            ...
                            : null;
                        numRaters = data.aggregateRating.ratingCount || null;
                        break;
                    }
                } catch {}
            }

            // Fallback: twitter meta tags
            if (!rating) {
                const meta = doc.querySelector('meta[name="twitter:data2"]');
                if (meta) {
                    const m = meta.getAttribute('content').match(/([\d.]+)/);
...
(async () => {
    ...
    if (host === 'movie.douban.com') {
        ...
        // Douban stops linking to IMDb, so find the text node instead and restore the link.
        ...
        // Fetch IMDb info and Letterboxd info in parallel
        const [data, lb_data] = await Promise.all([getIMDbInfo(id), getLetterboxdInfo(id)]);

        if (!data)
            return;
        if (isEmpty(data.imdbRating) && isEmpty(data.Metascore) && !lb_data) {
        ...
        if (lb_data) {
            ...
            const lb_voters = lb_data.numRaters
                ...
                : '?';
            insertDoubanRatingDiv(ratings, 'Letterboxd评分', lb_rating_10, lb_data.url, lb_voters);
        }
        ...
        // Fetch Douban and Letterboxd in parallel
        const [data, lb_data] = await Promise.all([getDoubanInfo(id), getLetterboxdInfo(id)]);
        ...
        // Insert Letterboxd rating (shown before Douban, so insert beforebegin Douban or IMDb)
        if (lb_data) {
            let lb_rating = imdb_rating.cloneNode(true);
            lb_rating.firstElementChild.textContent = 'Letterboxd RATING';
            lb_rating.children[1].href = lb_data.url;
            ...
            lb_rating_div.nextElementSibling.nextElementSibling.textContent = lb_data.numRaters
                ...
                : '';
            // Insert before Douban (which was inserted before IMDb), so LB ends up leftmost
            imdb_rating.previousElementSibling
                ? imdb_rating.previousElementSibling.insertAdjacentElement('beforebegin', lb_rating)
                : imdb_rating.insertAdjacentElement('beforebegin', lb_rating);
        }
